...
static uint32_t lora_bridge_boot_id = 0;
...
inline size_t encrypt_packet(
    ...
    // JAVÍTVA: A lokális static esp_random() helyett a globálisan/statikusan kezelt ID-t használjuk
...
bool decrypt_packet(const uint8_t* raw_data, size_t raw_len, LoRaPacket& decoded) {
    uint8_t decrypted_payload[256];
    size_t decrypted_len = sizeof(decrypted_payload);
    
    // Kicsomagoljuk a lokális változókba a nyers pointer és hossz alapján (0 heap!)
    bool success = AESGCMHelperV6::decrypt(
        raw_data, raw_len,                      // Nyers pointer és méret közvetlenül átadva
    ...
    if (success) {
        // Ha a decoded.payload továbbra is std::string, akkor ez az egyetlen pont, ami másol.
        // De ha a LoRaPacket struktúrában is átírod char payload[256]-ra, akkor ez is 0 heap lesz!
